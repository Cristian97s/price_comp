...
async def scrape_store_004(product_name: str, db):
    """
    Obteniendo productos desde La primera tienda usando Playwright.
    Devuelve una lista de dicts con: name, price, url, store
    """
    result = await db.execute(select(Store).filter(Store.id == 4))
    store = result.scalars().first()
    if not store:
        raise ValueError("No se encontró la tienda con id 4")

    search_url = f"{store.url}catalogsearch/result/{product_name.replace(' ', '+')}"
    results = []
    ...
    async with async_playwright() as p:
        ...
        await page.wait_for_selector("products.wrapper.grid.products-grid", timeout=20000)
